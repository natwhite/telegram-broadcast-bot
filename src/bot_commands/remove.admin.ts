...
export async function RemoveAdmin(context: Context) {
  ...
  DataService.removeAdmins(mentionedUsers);
  await DataService.saveDatabase();

  let message = `Removing users ${mentionedUsers.map(user => user.first_name).join(', ')} from the list of global admins`;
  console.log(message);
